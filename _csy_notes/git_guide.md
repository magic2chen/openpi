...
当你更换到新电脑（如从学校/公司电脑换到家里电脑），或者需要多台设备轮流切换学习时，请完整套用以下流程。

---

## 阶段一：在新电脑上首次初始化（一次性配置）

在一台全新的电脑上，执行以下 3 步来拉取你的所有笔记并建立通道：
...
1. **克隆你自己的 Fork 仓库**：
   ...
   ```
2. **切换到你的专属学习分支**：
   刚克隆下来时，本地默认处于 `main` 分支。直接切换到你的学习分支，Git 会自动把云端的所有笔记同步到本地：
   ```bash
   cd 项目名
   git checkout openpi-study
   ```
   * *检查：此时在本地就能看到你的 `_csy_notes` 文件夹和所有 Jupyter Notebook 笔记了。*
3. **重新关联原作者仓库（重新绑定上游）**：
   因为 `git clone` 默认只绑定了你自己的仓库（`origin`），必须重新把原作者的仓库添加为上游（`upstream`），以便后续同步官方更新：
   ```bash
   git remote add upstream https://github.com
   ```
...
## 阶段二：跨设备轮流切换的“黄金法则”（核心日常）

为了避免多台电脑由于进度不一致产生诡异的代码冲突，请务必养成以下两个无缝衔接的习惯：

### 🛑 习惯 A：在当前电脑（设备 A）结束学习时
离开电脑前，**务必将所有改动和笔记推送到云端**：
```bash
git add .
git commit -m "update: 记录在设备 A 上的最新进展"
git push
```

### 🚀 习惯 B：在另一台电脑（设备 B）开始学习前
坐到新电脑前，写任何代码、记任何笔记之前，**第一件事永远是拉取云端最新进度**：
```bash
# 确保在你的学习分支上
git checkout openpi-study

# 从你自己的云端仓库拉取最新笔记
git pull origin openpi-study
```
...
### Q1：如果我在设备 A 忘记 push 就回家了，在设备 B 写了新笔记怎么办？
* **情况**：两边进度错开了。
* **解决**：如果你严格按照要求把所有的 Jupyter 笔记都放在原项目不存在的 `_csy_notes/` 专属文件夹内，那么你在设备 B 提交并 push 后，回到设备 A 再次 `git pull` 时，Git 会**自动将两个不同的笔记文件合并**，绝不会损坏原项目的核心代码。你只需要处理好两边各自新增的文件即可。
